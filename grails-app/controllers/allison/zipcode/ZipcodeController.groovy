package allison.zipcode

import org.springframework.dao.DataIntegrityViolationException

class ZipcodeController {

    static allowedMethods = [save: "POST", update: "POST", delete: "POST"]

    def index() {
        redirect(action: "list", params: params)
    }

    def list(Integer max) {
        params.max = Math.min(max ?: 10, 100)
        [zipcodeInstanceList: Zipcode.list(params), zipcodeInstanceTotal: Zipcode.count()]
    }

    def create() {
        [zipcodeInstance: new Zipcode(params)]
    }

    def save() {
        def zipcodeInstance = new Zipcode(params)
        if (!zipcodeInstance.save(flush: true)) {
            render(view: "create", model: [zipcodeInstance: zipcodeInstance])
            return
        }

        flash.message = message(code: 'default.created.message', args: [message(code: 'zipcode.label', default: 'Zipcode'), zipcodeInstance.id])
        redirect(action: "show", id: zipcodeInstance.id)
    }

    def show(Long id) {
        def zipcodeInstance = Zipcode.get(id)
        if (!zipcodeInstance) {
            flash.message = message(code: 'default.not.found.message', args: [message(code: 'zipcode.label', default: 'Zipcode'), id])
            redirect(action: "list")
            return
        }

        [zipcodeInstance: zipcodeInstance]
    }

    def edit(Long id) {
        def zipcodeInstance = Zipcode.get(id)
        if (!zipcodeInstance) {
            flash.message = message(code: 'default.not.found.message', args: [message(code: 'zipcode.label', default: 'Zipcode'), id])
            redirect(action: "list")
            return
        }

        [zipcodeInstance: zipcodeInstance]
    }

    def update(Long id, Long version) {
        def zipcodeInstance = Zipcode.get(id)
        if (!zipcodeInstance) {
            flash.message = message(code: 'default.not.found.message', args: [message(code: 'zipcode.label', default: 'Zipcode'), id])
            redirect(action: "list")
            return
        }

        if (version != null) {
            if (zipcodeInstance.version > version) {
                zipcodeInstance.errors.rejectValue("version", "default.optimistic.locking.failure",
                          [message(code: 'zipcode.label', default: 'Zipcode')] as Object[],
                          "Another user has updated this Zipcode while you were editing")
                render(view: "edit", model: [zipcodeInstance: zipcodeInstance])
                return
            }
        }

        zipcodeInstance.properties = params

        if (!zipcodeInstance.save(flush: true)) {
            render(view: "edit", model: [zipcodeInstance: zipcodeInstance])
            return
        }

        flash.message = message(code: 'default.updated.message', args: [message(code: 'zipcode.label', default: 'Zipcode'), zipcodeInstance.id])
        redirect(action: "show", id: zipcodeInstance.id)
    }

    def delete(Long id) {
        def zipcodeInstance = Zipcode.get(id)
        if (!zipcodeInstance) {
            flash.message = message(code: 'default.not.found.message', args: [message(code: 'zipcode.label', default: 'Zipcode'), id])
            redirect(action: "list")
            return
        }

        try {
            zipcodeInstance.delete(flush: true)
            flash.message = message(code: 'default.deleted.message', args: [message(code: 'zipcode.label', default: 'Zipcode'), id])
            redirect(action: "list")
        }
        catch (DataIntegrityViolationException e) {
            flash.message = message(code: 'default.not.deleted.message', args: [message(code: 'zipcode.label', default: 'Zipcode'), id])
            redirect(action: "show", id: id)
        }
    }
}
