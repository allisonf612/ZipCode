			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="totalResultsCount" title="${message(code: 'state.totalResultsCount.label', default: 'Total Results Count')}" />
					
						<g:sortableColumn property="abbreviation" title="${message(code: 'state.abbreviation.label', default: 'Abbreviation')}" />
					
						<g:sortableColumn property="fullName" title="${message(code: 'state.fullName.label', default: 'Full Name')}" />
				...
				<g:each in="${stateInstanceList}" status="i" var="stateInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${stateInstance.id}">${fieldValue(bean: stateInstance, field: "totalResultsCount")}</g:link></td>
					
						<td>${fieldValue(bean: stateInstance, field: "abbreviation")}</td>
					
						<td>${fieldValue(bean: stateInstance, field: "fullName")}</td>
